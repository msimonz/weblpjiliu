...
export default function AdminPage() {
  ...
  async function uploadExcel() {
    ...
    try {
      ...
      // apiFetch usa JSON; para FormData usamos fetch directo:
  ...
  return (
    <div className="container">
      ...
      <div style={{ marginTop: 18, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        ...
        <div className="card">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 180px", gap: 12 }}>
            <div>
              ...
              <input className="input" value={newCourseName} onChange={(e) => setNewCourseName(e.target.value)} placeholder="Ej: Primer año - Curso1" />
            </div>
          ...
          <div style={{ marginTop: 12, overflow: "hidden", borderRadius: 18, border: "1px solid var(--stroke)" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {courses.length === 0 ? (
                  <tr><td colSpan={2} style={{ padding: 12, color: "var(--muted)" }}>{loadingData ? "Cargando..." : "Sin courses"}</td></tr>
                ) : courses.map((c) => (
        ...
        <div className="card">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 180px", gap: 12 }}>
            <div>
              <div className="label">Nombre</div>
              <input className="input" value={newClassName} onChange={(e) => setNewClassName(e.target.value)} placeholder="Ej: ETM - Nivel 1" />
            </div>
          ...
          <div style={{ marginTop: 12, overflow: "hidden", borderRadius: 18, border: "1px solid var(--stroke)" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {classes.length === 0 ? (
                  <tr><td colSpan={2} style={{ padding: 12, color: "var(--muted)" }}>{loadingData ? "Cargando..." : "Sin materias"}</td></tr>
                ) : classes.map((c) => (
                  <tr key={c.id} style={{ borderTop: "1px solid rgba(2,132,199,.10)" }}>
                    <td style={{ padding: 12, fontWeight: 900 }}>{c.name}</td>
                    <td style={{ padding: 12 }}>{c.level}</td>
        ...
        <div className="card">
          ...
          <div>
            ...
            <input className="input" value={newType} onChange={(e) => setNewType(e.target.value)} placeholder="Ej: Quiz, Parcial, Final..." />
          </div>
          ...
          <div style={{ marginTop: 12, overflow: "hidden", borderRadius: 18, border: "1px solid var(--stroke)" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {types.length === 0 ? (
                  <tr><td colSpan={2} style={{ padding: 12, color: "var(--muted)" }}>{loadingData ? "Cargando..." : "Sin tipos"}</td></tr>
                ) : types.map((t) => (
      ...
      {/* Asignar alumnos a course (ancho completo) */}
      <div className="card" style={{ marginTop: 18 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "320px 1fr 220px", gap: 12, alignItems: "end" }}>
          ...
          <div>
            ...
            <input className="input" value={studentQuery} onChange={(e) => setStudentQuery(e.target.value)} placeholder="Nombre, email o cédula..." />
          </div>
      ...
      {/* Subir Excel (ancho completo) */}
      <div className="card" style={{ marginTop: 18 }}>
        <h2 style={{ marginTop: 0 }}>Subir Excel: crear usuarios</h2>

        <div style={{ color: "var(--muted)", fontSize: 13, marginTop: 6 }}>
          Columnas esperadas: <b>email</b>, <b>name</b>, <b>type</b> (S/T/A), <b>cedula</b> (opcional), <b>id_course</b> (opcional), <b>code_jiliu</b> (opcional).
          <br />
