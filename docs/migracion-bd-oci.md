...
- [x] Probar en QA/staging. Deploy en QA (Render) confirmado por Alex: levanta bien. Validación a fondo desde este entorno: confirmado que un JWT emitido localmente es válido contra el backend real de QA (mismo `JWT_SECRET`, misma BD). Creados 5 usuarios de prueba reales vía API (uno por rol: A/T/M/E/S) y recorridos los 5 paneles en un navegador real contra `https://qa-sofialapromesa.onrender.com` — todos cargan con datos reales, header con nombre/rol correcto, **cero errores de consola**. Usuarios de prueba eliminados al final, sin dejar residuos.
...
- **2026-07-04**: Corregido `FRONTEND_URL` en Render (DEV/QA) a `https://qa-sofialapromesa.onrender.com` — es el dominio real del frontend (`qa-belapromesaxjiliu.onrender.com` es el backend). `CORS_ORIGINS` quedó con ambos dominios, sin problema. Variables de entorno del backend y frontend DEV/QA en Render cargadas por Alex (`DB_*`, `JWT_*`, `SMTP_*`, `FRONTEND_URL`; limpiadas `NEXT_PUBLIC_SUPABASE_*`/`NEXT_PUBLIC_USERS_TEMPLATE_URL` del frontend).
...
- **2026-07-04**: Validación a fondo de QA desde este entorno (tiene conectividad directa a `qa-belapromesaxjiliu.onrender.com` y `qa-sofialapromesa.onrender.com`). 5 usuarios de prueba por rol, recorrido completo en navegador real, cero errores de consola, limpieza total al final. Queda pendiente de la Fase 5: decidir fecha/forma del cutover a producción y período de respaldo de Supabase.
